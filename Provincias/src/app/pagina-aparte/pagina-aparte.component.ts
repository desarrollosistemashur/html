...
export class PaginaAparteComponent implements OnInit {
 ...
 public pagina: boolean=true;
  ...
  apiMunicipio(){
    fetch(`https://apis.datos.gob.ar/georef/api/municipios?provincia=${this.prov.id}&max=2000`)
      .then(response=>response.json())
      .then(municipioApi => this.municipioApi=municipioApi.municipios)
